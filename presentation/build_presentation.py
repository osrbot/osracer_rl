...
def build():
    ...
    s = new(); image(s, "site/source_assets/isaac-race.png", 7.15, 0, 6.18, 7.5)
    ...
    image(s, "site/source_assets/openusd-preview.png", .7, 2.05, 5.65, 3.18); image(s, "site/source_assets/mujoco-preview.png", 6.95, 2.05, 5.65, 3.18)
    ...
    s = new(); title(s, "ASSET TOOLCHAIN", "从 SolidWorks 到 URDF、OpenUSD 与 MJCF：导出不是黑箱")
    text(s, "资产由 SolidWorks URDF Exporter Pro 导出。它减少描述资产的重复配置，但不会自动替代控制器、摩擦参数或目标仿真器验证。", .72, 1.92, 11.8, .45, 15, INK)
    cards = [("多目标导出", "ROS 1 / ROS 2 URDF\nOpenUSD robot.usd\nMuJoCo robot.xml / scene.xml", TEAL), ("模型可审查", "链接树、关节、坐标系\n质量、质心、惯量与方向\n导出报告保留诊断", BLUE), ("碰撞与规模", "原语 / 包围盒 / 凸包 / 简化网格\n分区 STL 与实测文件大小反馈\n外观和碰撞可分开取舍", ORANGE), ("持续改进", "共享命名与元数据持久化\n旧配置迁移与单目标失败诊断\n欢迎 Issue、建议、测试和代码贡献", NAVY)]
    for i, (head, body, col) in enumerate(cards):
        x = .7 + (i % 2) * 6.05; y = 2.55 + (i // 2) * 1.75
        box(s, x, y, 5.55, 1.38, RGBColor(255,255,255), col, True); text(s, head, x+.24, y+.20, 2.2, .3, 18, col, True); text(s, body, x+.25, y+.58, 4.95, .65, 13, INK)
    text(s, "需要导出 OpenUSD、MJCF 或 URDF 做仿真时，欢迎试用：https://github.com/osrbot/solidworks_urdf_exporter_pro", .72, 6.25, 11.8, .35, 14, NAVY, True); footer(s, 4)
    ...
    bullet(s,"不使用全局位置；录像、轨迹和独立审计必须相互对应。", .9,5.25,11.4); footer(s, 5)
    ...
    text(s,"“加载模型”“单个成功视频”或“测试通过”都不能替代整圈资格。", .7,6.05,11.3,.35,18,RED,True); footer(s,6)
    ...
    box(s,9.2,2.25,3.1,3.45,RGBColor(255,244,240),ORANGE,True); text(s,"Isaac 的两条失败",9.45,2.58,2.5,.3,17,ORANGE,True); bullet(s,"Spa：起步车车接触",9.42,3.15,2.5); bullet(s,"Suzuka：桥面段车身失稳",9.42,3.82,2.5); text(s,"所以不能称 Isaac 全赛道完成。",9.45,4.75,2.35,.45,13,RED,True); footer(s,7)
    ...
    text(s,"工程结论：共享冻结配置维持 7.25 m/s；9.0 m/s 是 MuJoCo 速度榜，不是跨引擎稳定配置。", .85,6.25,11.35,.35,16,RED,True); footer(s,8)

    s = new(); title(s, "OVERTAKE", "主动超车已计入整圈资格，但只在明确的对手条件下成立")
    image(s,"site/source_assets/mujoco-race.png",.7,2.05,5.9,3.45); image(s,"site/source_assets/isaac-race.png",6.77,2.05,5.85,3.45)
    text(s,"MuJoCo Bahrain：代表样例",.75,5.62,4.0,.25,14,TEAL,True); text(s,"Isaac Bahrain：代表样例",6.8,5.62,4.0,.25,14,BLUE,True)
    text(s,"默认对手：同类传感器策略、巡航 2.8 m/s、发车领先 3 m。每条有效圈的“1 次”是完整超越，不代表对高速对手或任意初始位置的能力。",.75,6.12,11.65,.5,14,INK); footer(s,9)

    s = new(); title(s, "DRIFT", "180° 回头弯：仿真有漂移样例，但当前实车等效动作空间不支持它")
    image(s,"site/source_assets/drift.png",.75,2.0,5.75,3.55)
    ...
    bullet(s,"不证实单电机四驱实车能复现：其后轮增速等效为 0。",7.1,5.1,4.9,RED); footer(s,10)

    s = new(); title(s, "ROBUSTNESS", "主动观测还不是鲁棒感知：组合扰动下 10/10 失败")
    image(s,"site/source_assets/perturb.png",.75,2.0,5.75,3.55)
    ...
    bullet(s,"根因：起步扫描匹配失败，进入 motion_estimate_unavailable 的盲车模式。",7.1,5.2,4.9,RED); footer(s,11)
    ...
    text(s,"不要跳过阶段：两条“real”观测仿真初测有效，不等价于任何实车运行。",.7,5.75,11.5,.35,17,RED,True); footer(s,12)
    ...
    text(s,"谢谢。欢迎质疑结论，但请连同实验条件一起质疑。",.8,6.25,11.4,.35,18,INK,True); footer(s,13)
